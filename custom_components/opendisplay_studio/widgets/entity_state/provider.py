"""Home Assistant data provider owned by the Entity State widget package."""
...
class EntityStateProvider:
    ...
    def get_many(self, entity_ids: set[str]) -> dict[str, dict[str, str]]:
        """Return normalized values for every requested entity."""
...
class EntityStateDataProvider:
    """Generic aggregate provider for entity-state widget requirements."""
    ...
    def add_request(
        ...
        """Add selected entity IDs to the aggregate request."""
    ...
    async def async_resolve(
        ...
        """Resolve current states for every aggregate entity ID."""
        del language
        return EntityStateProvider(hass).get_many(cast("set[str]", request))
    ...
    def values(
        ...
        """Map normalized entity states back to one widget."""
        entities = cast("dict[str, dict[str, str]]", resolved)
        values = [
            dict(
                entities.get(
                    source,
        ...
        for value in values:
            value["displayName"] = title or value["name"]
        return values


PROVIDER = EntityStateDataProvider()
